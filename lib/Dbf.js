...
Dbf.prototype.forEach = function(iterator, done) {
	var self = this;
	this._promise.then(
		function() {
			self._forEach(iterator, done);
		},
		function(err) {
			done(err);
		}
...
Dbf.prototype._forEach = function(iterator, done) {

	var maxIndex = this._header.records, index = 0;

	if (maxIndex <= 0) {
		done(new Error('Invalid maxIndex in dbf'));
		return;
	}
	...
	function iterate(err, record) {
		if (err)
			...
			iterator(record, index, function(err) {
				if (_next) {
					_next = false;
					next(err);
				}
