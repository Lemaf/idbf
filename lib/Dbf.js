...
Dbf.prototype.forEach = function(callback) {
	var self = this;
	this._promise.then(
		function() {
			self._forEach(callback);
		},
		function(err) {
			callback(err);
		}
...
Dbf.prototype._forEach = function(callback) {

	var maxIndex = this._header.records, index = 0;

	if (maxIndex <= 0) {
		return;
	}
/*
	function iterator(index, maxIndex, callback){

		Dbf._get(index, function(err, record) {
			...
			if (err) {
				...
				if(index < maxIndex)
					iterator(index+1, maxIndex, callback);
	...
	function iterator(err, record) {
		if (err)
			callback(err)
		else {
			callback(null, record, index);

			if (index < maxIndex)
				self._get(index++, iterator)
		}
	}


	this._get(index, iterator);

	//iterator(0, maxIndex, callback);
