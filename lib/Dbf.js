...
Dbf.prototype.forEach = function(iterator) {
	var self = this;
	this._promise.then(
		function() {
			self._forEach(iterator);
		},
		function(err) {
			iterator(err);
		}
...
Dbf.prototype._forEach = function(iterator) {
	...
	function next() {
		...
		if (index < maxIndex)
			self._get(index, iterate)
		else
			iterator(null, null, null, null);
	}
	
	function iterate(err, record) {
		if (err)
			iterator(err, null, null, null);
		else {
			var _next = true
			// Secure next!
			iterator(null, record, index, function() {
				if (_next) {
					_next = false;
					next();
				}
